...
static function state($id){ 
    ...
    try {
        ...
        // 状態を更新
        $st = db()->prepare('UPDATE requests SET state = ? WHERE id = ?');
        ...
        if ($rowCount === 0) {
            error_log("No rows updated - checking if state is already the same");
            // 現在の状態を確認
            $checkCurrent = db()->prepare('SELECT state FROM requests WHERE id = ?');
            $checkCurrent->execute([$id]);
            $currentState = $checkCurrent->fetch();
            error_log("Current state in DB: '" . $currentState['state'] . "'");
            
            if ($currentState['state'] === $next) {
                error_log("State is already correct, continuing");
            } else {
                error_log("State mismatch - expected: '$next', actual: '" . $currentState['state'] . "'");
                return json_ng('状態の更新に失敗しました', 500);
            }
        }
        ...
        if (($next === 'CASH_GIVEN' || $next === 'TRANSFERRED') && $request['state'] === 'ACCEPTED') {
            try {
                // リクエスト情報を取得（expects_networkとcash_given）
                $reqStmt = db()->prepare('SELECT expects_network, cash_given FROM requests WHERE id = ?');
                $reqStmt->execute([$id]);
                $reqData = $reqStmt->fetch(PDO::FETCH_ASSOC);
                
                if ($reqData && $reqData['cash_given'] > 0) {
                    $amount = (float)$reqData['cash_given'];
                    $pdo = db();
                    
                    // 銀行振込の場合は銀行残高から減算（現金渡しの場合は持ち出し額から減算しない）
                    // 持ち出し額は銀行からの引き出し・預金のみで変動し、渡し額による減算は行わない
                    // 理論所持金額の計算で未処理の購入希望額（pendingAmount）として反映される
                    if ($reqData['expects_network'] === 'BANK_TRANSFER') {
                        // 現在のfundsテーブルの状態を取得
                        ...
                        error_log("Bank balance decreased by {$amount} for request {$id}. New balance: {$newBankBalance}. Cash on hand increased by {$amount}. New cash on hand: {$newCashOnHand}");
                    }
                    // 現金渡しの場合は持ち出し額から減算しない（理論所持金額の計算でpendingAmountとして反映される）
                }
            } catch (Exception $e) {
        ...
        // 振込済み（TRANSFERRED）から他の状態（ACCEPTEDなど）に戻された場合、銀行残高に戻し、持ち出し額から減算
        if ($request['state'] === 'TRANSFERRED' && $next !== 'TRANSFERRED' && $next !== 'COLLECTED' && $next !== 'RECEIPT_DONE' && $next !== 'FINALIZED') {
            try {
                ...
                if ($reqData && $reqData['cash_given'] > 0 && $reqData['expects_network'] === 'BANK_TRANSFER') {
                    $amount = (float)$reqData['cash_given'];
                    $pdo = db();
                    ...
                    error_log("Bank balance increased by {$amount} for request {$id} (state reverted from TRANSFERRED). New balance: {$newBankBalance}. Cash on hand decreased by {$amount}. New cash on hand: {$newCashOnHand}");
                }
            } catch (Exception $e) {
        ...
        // 更新後の状態を確認
        $verify = db()->prepare('SELECT state FROM requests WHERE id = ?');
        $verify->execute([$id]);
        $updatedState = $verify->fetch();
        error_log("Final state verification - ID: $id, State: '" . $updatedState['state'] . "'");
        
        error_log("State update successful - ID: $id, New state: $next");
        return json_ok();

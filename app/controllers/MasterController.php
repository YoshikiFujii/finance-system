...
class MasterController
{
    ...
    static function accounting_periods_list()
    {
        require_role(['OFFICER', 'FINANCE', 'ADMIN', 'AUDIT']);
    ...
    static function upsert_member()
    {
        ...
        if (!$name || !$dep)
            return json_ng('name and department_id required');
    ...
    static function delete_department()
    {
        ...
        if ($member_count > 0)
            return json_ng('部署に関連するメンバーが存在するため削除できません');
        ...
        if ($request_count > 0)
            return json_ng('部署に関連するリクエストが存在するため削除できません');
    ...
    static function delete_member()
    {
        ...
        $request_count = $st->fetchColumn();
        if ($request_count > 0)
            return json_ng('この役員に関連するリクエストが存在するため削除できません');
    ...
    static function upsert_event()
    {
        ...
        if ($name === '')
            return json_ng('name required');
        if ($id) {
            $st = db()->prepare('UPDATE events SET name=?, is_active=? WHERE id=?');
            $st->execute([$name, $active, $id]);
        } else {
            $st = db()->prepare('INSERT INTO events(name,is_active) VALUES(?,?)');
            $st->execute([$name, $active]);
        }
        return json_ok();
    }
    static function delete_event()
    {
        ...
        $id = (int) ($_GET['id'] ?? 0);
        if (!$id)
            return json_ng('id required', 400);
    ...
    static function delete_accounting_period()
    {
        ...
        if ($receipt_count > 0)
            return json_ng('計上期間に関連するレシートが存在するため削除できません');
    ...
    // 年度マスタ管理
    static function years_list()
    {
        require_role(['OFFICER', 'FINANCE', 'ADMIN', 'AUDIT']);
        $rows = db()->query('SELECT id,name,is_active FROM years ORDER BY id DESC')->fetchAll();
        return json_ok($rows);
    }

    static function upsert_year()
    {
        require_role(['ADMIN']);
        $in = json_decode(file_get_contents('php://input'), true) ?: [];
        ...
        // 削除アクションの処理
        if (isset($in['action']) && $in['action'] === 'delete' && isset($in['id'])) {
            return self::delete_year();
        }

        $id = $in['id'] ?? null;
        $name = trim((string) ($in['name'] ?? ''));
        $active = (int) ($in['is_active'] ?? 0);
        ...
        // activeで作成する場合、他のすべての期間をinactiveにする
        if ($active == 1) {
            $st = db()->prepare('UPDATE years SET is_active = 0');
            $st->execute();
        }

        if ($id) {
            $st = db()->prepare('UPDATE years SET name=?, is_active=? WHERE id=?');
            $st->execute([$name, $active, $id]);
        } else {
            $st = db()->prepare('INSERT INTO years(name,is_active) VALUES(?,?)');
            $st->execute([$name, $active]);
        }
        return json_ok();
    }

    static function delete_year()
    {
        require_role(['ADMIN']);
        $in = json_decode(file_get_contents('php://input'), true) ?: [];
        // POSTのaction=delete経由も、DELETEメソッド直接呼び出しも対応
        $id = (int) ($in['id'] ?? 0);
        if (!$id)
            return json_ng('id required');

        // 関連チェック
        $cnt = db()->prepare('SELECT COUNT(*) FROM receipts WHERE year_id=?');
        $cnt->execute([$id]);
        if ($cnt->fetchColumn() > 0)
            return json_ng('関連するレシートがあるため削除できません');

        $cnt = db()->prepare('SELECT COUNT(*) FROM requests WHERE year_id=?');
        $cnt->execute([$id]);
        if ($cnt->fetchColumn() > 0)
            return json_ng('関連するリクエストがあるため削除できません');

        $st = db()->prepare('DELETE FROM years WHERE id=?');
        $st->execute([$id]);
        return json_ok();
    }
}